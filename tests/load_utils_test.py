...
def test_load_imgs_from_mibitiff():
    ...
    with pytest.raises(FileNotFoundError):
        loaded_xr = load_utils.load_imgs_from_mibitiff("not_a_dir", channels=None, delimiter="_")

    with tempfile.TemporaryDirectory() as temp_dir:
        # temp_dir contains no images
        with pytest.raises(ValueError):
            loaded_xr = load_utils.load_imgs_from_mibitiff(temp_dir, channels=None, delimiter="_")
        ...
        with pytest.raises(ValueError):
            ...
            loaded_xr = load_utils.load_imgs_from_mibitiff(temp_dir, channels=[], delimiter="_")

        # check unspecified fov loading
        loaded_xr = load_utils.load_imgs_from_mibitiff(temp_dir, channels=channels, delimiter="_")
        ...
        loaded_xr = load_utils.load_imgs_from_mibitiff(
            temp_dir, mibitiff_files=[fovnames[-1]], channels=channels, delimiter="_"
        )

        assert loaded_xr.equals(data_xr.loc[[fovs[-1]], :, :, :])

        # test automatic all channels loading
        loaded_xr = load_utils.load_imgs_from_mibitiff(temp_dir, delimiter="_")

        assert loaded_xr.equals(data_xr)

        # test delimiter agnosticism
        loaded_xr = load_utils.load_imgs_from_mibitiff(
            temp_dir, mibitiff_files=fovnames, channels=channels, delimiter="_"
        )
...
def test_load_imgs_from_tree():
    # invalid directory is provided
    with pytest.raises(FileNotFoundError):
        loaded_xr = load_utils.load_imgs_from_tree("not_a_dir", img_sub_folder="TIFs")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with pytest.raises(ValueError):
            # attempt to pass an empty channels list
            loaded_xr = load_utils.load_imgs_from_tree(temp_dir, img_sub_folder="TIFs", channels=[])
        ...
        loaded_xr = load_utils.load_imgs_from_tree(
            temp_dir, img_sub_folder="TIFs", fovs=some_fovs, channels=some_imgs
        )
        ...
        loaded_xr = load_utils.load_imgs_from_tree(
            temp_dir, img_sub_folder="TIFs", channels=some_chans
        )
        ...
        loaded_xr = load_utils.load_imgs_from_tree(
            temp_dir, img_sub_folder="TIFs", fovs="fov0", channels=some_chans
        )
        ...
        with pytest.raises(ValueError):
            loaded_xr = load_utils.load_imgs_from_tree(
                temp_dir, img_sub_folder="TIFs", channels=["chan4"]
            )

    # test loading with data_xr containing float values
    with tempfile.TemporaryDirectory() as temp_dir:
        fovs, chans, imgs = test_utils.gen_fov_chan_names(num_fovs=1, num_chans=2, return_imgs=True)
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        loaded_xr = load_utils.load_imgs_from_tree(
            temp_dir, img_sub_folder="TIFs", max_image_size=12
        )
...
def test_load_imgs_from_dir():
    # invalid directory is provided
    with pytest.raises(FileNotFoundError):
        loaded_xr = load_utils.load_imgs_from_dir("not_a_dir", trim_suffix="_")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        filelocs, data_xr = test_utils.create_paired_xarray_fovs(
            temp_dir, fovs, [0], img_shape=(10, 10), mode="labels", delimiter="_", dtype=np.float32
        )

        # invalid list of files is provided
        with pytest.raises(ValueError):
            load_utils.load_imgs_from_dir(temp_dir, files=fovs + ["not_an_image"], trim_suffix="_")
        with pytest.raises(ValueError):
            load_utils.load_imgs_from_dir(temp_dir, files=["not_an_image"], trim_suffix="_")

        # check default loading
        loaded_xr = load_utils.load_imgs_from_dir(
            temp_dir, trim_suffix="_", xr_dim_name="compartments"
        )
        ...
        loaded_xr = load_utils.load_imgs_from_dir(
            temp_dir, match_substring="_otherinfo", trim_suffix="_", xr_dim_name="compartments"
        )
        ...
        loaded_xr = load_utils.load_imgs_from_dir(
            temp_dir, match_substring="ov", trim_suffix="_", xr_dim_name="compartments"
        )
        ...
        with pytest.raises(ValueError):
            load_utils.load_imgs_from_dir(
                temp_dir,
                match_substring="not_a_real_suffix",
                trim_suffix="_",
                xr_dim_name="compartments",
            )
    ...
    for dtype in [np.float32, np.int16]:
        with tempfile.TemporaryDirectory() as temp_dir:
            fovs, _ = test_utils.gen_fov_chan_names(num_fovs=3, num_chans=0, use_delimiter=True)
            filelocs, data_xr = test_utils.create_paired_xarray_fovs(
                temp_dir, fovs, [0], img_shape=(10, 10), mode="labels", delimiter="_", dtype=dtype
            )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        loaded_xr = load_utils.load_imgs_from_dir(
            temp_dir, files=[fovnames[-1]], xr_dim_name="channels", trim_suffix="_"
        )
        assert loaded_xr.equals(data_xr.loc[[fovs[-1]], :, :, :])
        ...
        loaded_xr = load_utils.load_imgs_from_dir(
            temp_dir, files=None, channel_indices=None, xr_dim_name="channels", trim_suffix="_"
        )
        ...
        loaded_xr = load_utils.load_imgs_from_dir(
            temp_dir, files=None, channel_indices=[0, 1, 2], xr_dim_name="channels", trim_suffix="_"
        )
...
class TestOMEConversion:
    ...
    @pytest.mark.parametrize(
        "fovs", [["fov0"], ["fov0", "fov1"], None], ids=["single_fov", "multiple_fovs", "all_fovs"]
    )
    @pytest.mark.parametrize(
        "channels",
        [["chan0"], ["chan0", "chan1", "chan3"], None],
        ids=["single_channel", "multiple_channels", "all_channels"],
    )
    def test_fov_to_ome(self, fovs, channels) -> None:
        load_utils.fov_to_ome(
            data_dir=self.test_dir, ome_save_dir=self.save_path, fovs=fovs, channels=channels
        )
