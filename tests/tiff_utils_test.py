...
def test_read_mibitiff():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        filepaths, true_data = test_utils.create_paired_xarray_fovs(
            temp_dir,
            fov_names=fovs,
            img_shape=(1024, 1024),
            channel_names=chans,
        ...
        subset_img_data, subset_chan = tiff_utils.read_mibitiff(
            filepaths[fovs[0]], channels=chan_names[:3]
        )
...
def test_write_mibitiff():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        filepaths, true_data = test_utils.create_paired_xarray_fovs(
            temp_dir, fov_names=fovs, channel_names=chans, mode="mibitiff", fills=True
        )
